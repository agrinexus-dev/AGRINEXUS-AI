...
import { useRobotMissionStore, useSelectedRobotMission } from "@/lib/robot-missions/robot-mission-store";
import { ROBOT_MISSION_ACTIVE_STATUSES, ROBOT_MISSION_STATUS_LABELS, ROBOT_MISSION_TYPE_LABELS, type RobotMissionStatus } from "@/lib/robot-missions/types";
...
export function RobotMissionInspector() {
  const mission = useSelectedRobotMission();
  const robots = useRobots();
  const store = useRobotMissionStore();
  ...
  // Mirrors `mission-inspector.tsx`'s own
  // `inspectionEnabled` exactly; "Manual Drive" is the one robot mission
  // type that skips scanning (a human-driven robot isn't running the
  // automated work pattern proximity detection relies on).
  const inspectionEnabled = mission.missionType !== "manual-drive" && Boolean(mission.targetPlotId);
  const isQueued = mission.status === "queued";
  const canGenerateRoute = isQueued && Boolean(mission.assignedRobotId);
  const canStart = isQueued && mission.waypoints.length > 0;
  const canPause = ROBOT_MISSION_ACTIVE_STATUSES.includes(mission.status);
  ...
  return (
    <div className="flex h-full flex-col gap-3 overflow-y-auto">
      <Card>
          ...
          <div className="flex flex-col gap-1">
            ...
            {inspectionEnabled ? (
              <Typography variant="caption" className="text-foreground-subtle">
                Inspection scanning active — this mission scans {plot?.label ?? "its target plot"} for crop issues, and can act on supported ones.
              </Typography>
            ) : null}
            {recurringConfig ? (
        ...
        <CardContent className="flex flex-col gap-2.5">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <Select
              value={mission.assignedRobotId ?? "__none"}
              onValueChange={(value) => store.assignRobot(mission.id, value === "__none" ? null : value)}
              disabled={!isQueued}
            >
              <SelectTrigger>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Drive Parameters</CardTitle>
        </CardHeader>
        <CardContent className="flex flex-col gap-3">
          <div className="grid grid-cols-2 gap-3">
            ...
            <NumberField
              label="Path Spacing %"
              value={mission.pathSpacingPercent}
              disabled={!isQueued}
              onChange={(value) => store.updateMissionSettings(mission.id, { pathSpacingPercent: value })}
            />
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent className="flex flex-col gap-2.5">
          {mission.estimate ? (
            <>
              ...
              <InspectorRow label="Distance" value={`${mission.estimate.distanceMeters} m`} />
              ...
              {mission.headingDegrees !== null ? <InspectorRow label="Robot Heading" value={`${Math.round(mission.headingDegrees)}°`} /> : null}
            </>
          ) : (
            <Typography variant="small">Generate a ground route to see coverage, distance, battery, and time estimates.</Typography>
          )}
        </CardContent>
      </Card>
      ...
      <div className="flex flex-col gap-2">
        <Button intent="secondary" leadingIcon={<Route />} disabled={!canGenerateRoute} onClick={() => store.generateRoute(mission.id)}>
          Generate Route
        </Button>
...
/** Mirrors `mission-inspector.tsx`'s own `InspectionFindingsCard` exactly; the extra line here (`correctiveActionDescription` doubling as a "resolved" line, same as the drone version) is what shows a robot's simulated corrective action once performed. */
